      ...
      <ul class="slider_area" @mouseover="stop" @mouseout="play">
          <template v-if="pollens.length > 0">
              <li v-for="(image,index) in pollens" :style="activeImageScale(index)">
                  ...
                  <div class="desc els" :title="image.name">{{ image.name }}</div>
              </li>
          </template>
...
export default {
    ...
    created: function () { // 这里mounted和created生命周期函数区别
        this.$http.get('http://localhost:8081/search/pollens')
            .then(function (response) {
                this.pollens = response.body.data;
            })
...
.slider_area .arrow.fixed,
.slider_area:hover .arrow.hover {
  opacity: 1;
  cursor: pointer;
}
